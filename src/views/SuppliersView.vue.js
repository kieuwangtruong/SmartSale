...
import { createSupplier, deleteSupplier, getSuppliers, updateSupplier } from '../services/orderApi';
import { useAuthStore } from '../stores/authStore';
const auth = useAuthStore();
const canManageSuppliers = computed(() => auth.role === 'Admin' || auth.role === 'WarehouseKeeper');
const suppliers = ref([]);
...
const filtered = computed(() => {
    ...
    return !q ? suppliers.value : suppliers.value.filter((s) => [s.name, s.contactName, s.phone, s.email, s.address, s.notes].some((val) => val && val.toLowerCase().includes(q)));
});
...
async function save() {
    try {
        ...
        error.value = e instanceof Error ? e.message : 'Không thể lưu nhà cung cấp.';
    }
}
...
if (__VLS_ctx.canManageSuppliers) {
    __VLS_asFunctionalElement1(__VLS_intrinsics.button, __VLS_intrinsics.button)({
        ...{ onClick: (...[$event]) => {
                ...
                // @ts-ignore
                [search, canManageSuppliers, showForm,];
            } },
        type: "button",
        ...{ class: "primary" },
    });
...
if (__VLS_ctx.showForm && __VLS_ctx.canManageSuppliers) {
...
if (__VLS_ctx.showForm && __VLS_ctx.canManageSuppliers) {
    __VLS_asFunctionalElement1(__VLS_intrinsics.aside, __VLS_intrinsics.aside)({
        ...{ class: "admin-modal" },
        'aria-label': "Biểu mẫu nhà cung cấp",
    });
    ...
    __VLS_asFunctionalElement1(__VLS_intrinsics.h2, __VLS_intrinsics.h2)({});
    (__VLS_ctx.editingId ? 'Cập nhật nhà cung cấp' : 'Thêm nhà cung cấp');
    __VLS_asFunctionalElement1(__VLS_intrinsics.button, __VLS_intrinsics.button)({
        ...{ onClick: (__VLS_ctx.reset) },
        type: "button",
    });
    ...
    if (__VLS_ctx.canManageSuppliers) {
        ...
        __VLS_asFunctionalElement1(__VLS_intrinsics.button, __VLS_intrinsics.button)({
            ...{ onClick: (...[$event]) => {
                    ...
                    __VLS_ctx.edit(item);
                    // @ts-ignore
                    [canManageSuppliers, canManageSuppliers, canManageSuppliers, canManageSuppliers, showForm, showForm, error, error, reset, reset, reset, editingId, save, form, form, form, form, form, form, visible, edit,];
                } },
        });
        __VLS_asFunctionalElement1(__VLS_intrinsics.button, __VLS_intrinsics.button)({
            ...{ onClick: (...[$event]) => {
